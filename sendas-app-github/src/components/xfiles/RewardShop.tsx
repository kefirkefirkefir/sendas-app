...
import { useGameStore, type Reward, REWARD_TEMPLATES } from "@/lib/game-store";
...
function getWeeklyDeal(seed?: number): { template: typeof REWARD_TEMPLATES[number]; originalCost: number; dealCost: number } {
  const now = new Date();
  const week = getISOWeek(now);
  const baseSeed = now.getFullYear() * 100 + week;
  const idx = seededIndex(seed !== undefined ? baseSeed + seed : baseSeed, REWARD_TEMPLATES.length);
  const template = REWARD_TEMPLATES[idx];
...
export default function RewardShop() {
  ...
  // Weekly deal — uses store seed for randomization on reset
  const weeklyDeal = useMemo(() => getWeeklyDeal(weeklyDealSeed), [weeklyDealSeed]);

  const handleTemplateSelect = (idx: string) => {
    setSelectedTemplate(idx);
    if (idx === "none") return;
    const t = REWARD_TEMPLATES[parseInt(idx)];
    if (!t) return;
    setName(t.name);
                  ...
                  {REWARD_TEMPLATES.map((t, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[10px] opacity-60">{t.cost} <Coins className="w-2.5 h-2.5 inline text-[#fbbf24]" /></span>
                      </button>
                    );
                  })}
                </div>
          ...
          {redeemedRewards.length === 0 ? (
            ...
            <AnimatePresence>
              {sortedRedeemed.map((red, idx) => {
                const reward = rewards.find((r) => r.id === red.id);
                ...
                return (
                  ...
                  >
                    {(() => { const Ic = getRewardIcon(reward.icon); return <Ic className="w-6 h-6" style={{ color: mc.accent }} />; })()}
                    <div className="flex-1 min-w-0">
                      <div className="font-mono text-xs text-[#6b8a6b] line-through">
                        {reward.name}
                      </div>
                      <div className="font-mono text-[10px] text-[#4a5a4a]">
                        {reward.cost} <Coins className="w-3 h-3 inline text-[#fbbf24]" /> canjeados
                      </div>
                      {red.redeemedAt && (
