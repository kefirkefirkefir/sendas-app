...
export interface GameState {
  ...
  // Rewards
  rewards: Reward[];
  redeemedRewards: { id: string; name: string; description: string; cost: number; icon: string; redeemedAt: string }[];
  ...
  persist(
    (set, get) => ({
      ...
      redeemReward: (id) => {
        ...
        // Consumible: se elimina del mercado y pasa al historial con sus datos
        set({
    ...
    }),
    {
      ...
      migrate: (persisted: any, version: number) => {
        ...
        // Migrate redeemedRewards: string[] → {id, redeemedAt}[] → {id, name, description, cost, icon, redeemedAt}[]
        if (Array.isArray(persisted.redeemedRewards)) {
          persisted.redeemedRewards = persisted.redeemedRewards.map((r: any) =>
            typeof r === "string"
              ? { id: r, name: "Recompensa", description: "", cost: 0, icon: "ShoppingBag", redeemedAt: new Date().toISOString() }
              : r.name
                ? r
                : { ...r, name: r.name || "Recompensa", description: r.description || "", cost: r.cost || 0, icon: r.icon || "ShoppingBag" }
          );
        }
