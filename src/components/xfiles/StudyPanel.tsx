...
import {
  ...
  type TemaFase, type TemaPrioridad, type TemaOposicion, type TemaIdioma, type TemaIdiomaHabilidad, type TemaAsignatura, type TemaAsignaturaTipo,
...
// Theme-aware dialog backgrounds are handled by mc.bg + CSS overrides.
...
type IdiomaHabilidad = "vocabulario" | "gramatica" | "escucha" | "conversacion" | "lectura" | "escritura";

const HABILIDAD_COLORS: Record<IdiomaHabilidad, string> = {
  vocabulario: "text-[#60a5fa]",
  gramatica: "text-[#a78bfa]",
  escucha: "text-[#f472b6]",
  conversacion: "text-[#fbbf24]",
  lectura: "text-[#34d399]",
  escritura: "text-[#fb923c]",
};

const HABILIDAD_BG_COLORS: Record<IdiomaHabilidad, string> = {
  vocabulario: "bg-[rgba(96,165,250,0.1)] border-[rgba(96,165,250,0.25)]",
  gramatica: "bg-[rgba(167,139,250,0.1)] border-[rgba(167,139,250,0.25)]",
  escucha: "bg-[rgba(244,114,182,0.1)] border-[rgba(244,114,182,0.25)]",
  conversacion: "bg-[rgba(251,191,36,0.1)] border-[rgba(251,191,36,0.25)]",
  lectura: "bg-[rgba(52,211,153,0.1)] border-[rgba(52,211,153,0.25)]",
  escritura: "bg-[rgba(251,146,60,0.1)] border-[rgba(251,146,60,0.25)]",
};

function getIdiomaAutoPriority(tema: TemaIdioma): AutoPriority {
  const diasSinEstudio = tema.fechaUltimoEstudio
    ? Math.floor((Date.now() - new Date(tema.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000)
    : 9999;

  if ((tema.fase === "no_visto" && diasSinEstudio > 7) || diasSinEstudio > 14 || tema.prioridad === "alta") {
    return "alta";
  }
  if ((tema.fase && tema.fase !== "no_visto" && tema.fase !== "dominado" && diasSinEstudio <= 7) || tema.prioridad === "media") {
    return "media";
  }
  return "baja";
}

function getIdiomaHabilidadStats(temas: TemaIdioma[]): Record<string, { total: number; dominados: number; enProgreso: number }> {
  const stats: Record<string, { total: number; dominados: number; enProgreso: number }> = {};
  for (const t of temas) {
    const h = t.habilidad || "vocabulario";
    if (!stats[h]) stats[h] = { total: 0, dominados: 0, enProgreso: 0 };
    stats[h].total++;
    if (t.completado || t.fase === "dominado") stats[h].dominados++;
    else if (t.fase && t.fase !== "no_visto") stats[h].enProgreso++;
  }
  return stats;
}

function getIdiomaSuggestedAction(idi: Idioma): React.ReactNode {
  if (idi.temas.length === 0) return null;
  const sorted = [...idi.temas]
    .filter((t) => !t.completado && t.fase !== "dominado")
    .sort((a, b) => {
      const pa = getIdiomaAutoPriority(a);
      const pb = getIdiomaAutoPriority(b);
  ...
  const next = sorted[0];
  if (!next) return null;
  const diasSinEstudio = next.fechaUltimoEstudio
    ? Math.floor((Date.now() - new Date(next.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000)
    : 9999;
  const habilidadLabel = HABILIDAD_LABELS[next.habilidad || "vocabulario"];
  if (!next.fechaUltimoEstudio) {
    return <><span className="text-[#60a5fa]">Estudia</span> <span className="text-[#c8d6c0]">{next.texto}</span> <span className="text-[#4a5a4a]">({habilidadLabel})</span></>;
  }
  if (diasSinEstudio > 7) {
    return <><span className="text-[#fbbf24]">Repasa</span> <span className="text-[#c8d6c0]">{next.texto}</span> <span className="text-[#fbbf24]">({diasSinEstudio}d sin tocar)</span></>;
  }
  if (next.fase === "aprendiendo") {
    return <><span className="text-[#a78bfa]">Practica</span> <span className="text-[#c8d6c0]">{next.texto}</span></>;
  }
  return <><span className="text-[#60a5fa]">Estudia</span> <span className="text-[#c8d6c0]">{next.texto}</span></>;
}

// ---- Curso helpers ----

function getTemaAsigAutoPriority(tema: TemaAsignatura): AutoPriority {
  const diasSinEstudio = tema.fechaUltimoEstudio
    ? Math.floor((Date.now() - new Date(tema.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000)
    : 9999;
...
function calcCursoStats(curso: Curso) {
  const aprobadas = curso.asignaturas.filter((a) => a.estado === "Aprobada" || a.estado === "Convalidada").length;
  const cursando = curso.asignaturas.filter((a) => a.estado === "Cursando").length;
  const suspensas = curso.asignaturas.filter((a) => a.estado === "Suspensa").length;
  const pendientes = curso.asignaturas.filter((a) => a.estado === "Pendiente").length;
  const totalECTS = curso.asignaturas.reduce((s, a) => s + a.creditos, 0);
  const ectsSuperados = curso.asignaturas.filter((a) => a.estado === "Aprobada" || a.estado === "Convalidada").reduce((s, a) => s + a.creditos, 0);
  const notas = curso.asignaturas.filter((a) => {
    const nf = calcNotaFinal(a.evaluaciones);
    return nf !== "---";
  });
  const notaMedia = notas.length > 0
    ? notas.reduce((s, a) => s + parseFloat(calcNotaFinal(a.evaluaciones)), 0) / notas.length
    : null;
  const enRiesgo = curso.asignaturas.filter((a) => {
    if (a.estado !== "Cursando") return false;
    const nf = calcNotaFinal(a.evaluaciones);
    if (nf === "---") return false;
    return parseFloat(nf) < 5;
  }).length;
  return { aprobadas, cursando, suspensas, pendientes, totalECTS, ectsSuperados, notaMedia, enRiesgo, total: curso.asignaturas.length };
}

function diasHastaExamen(fechaExamen: string): number | null {
  if (!fechaExamen) return null;
  const exam = new Date(fechaExamen + "T00:00:00");
  const now = new Date();
  now.setHours(0, 0, 0, 0);
...
export default function StudyPanel() {
  ...
  const {
    ...
    estudiarIdiomaTema, estudiarAsignaturaTema,
  ...
  // Idioma edit dialog
  const [editIdiomaDialogOpen, setEditIdiomaDialogOpen] = useState(false);

  // Curso edit dialog
  const [editCurDialogOpen, setEditCurDialogOpen] = useState(false);

  // Asignatura edit dialog
  const [editAsigDialogOpen, setEditAsigDialogOpen] = useState(false);

  // New tema habilidad selector
  const [newTemaIdiHabilidad, setNewTemaIdiHabilidad] = useState<IdiomaHabilidad>("vocabulario");
  ...
  function saveIdiomaFromDialog() {
    if (!selectedIdioma || !editIdiForm) return;
    updateIdioma(selectedIdioma, editIdiForm);
    setEditIdiomaDialogOpen(false);
    showToast("Idioma guardado");
  }

  function saveCursoFromDialog() {
    if (!selectedCurso || !editCurForm) return;
    updateCurso(selectedCurso, editCurForm);
    setEditCurDialogOpen(false);
    showToast("Curso guardado");
  }

  function saveAsignaturaFromDialog() {
    if (!selectedCurso || !selectedAsignatura || !editAsigForm) return;
    updateAsignatura(selectedCurso, selectedAsignatura, editAsigForm);
    setEditAsigDialogOpen(false);
    showToast("Asignatura guardada");
  }

  function handleEstudiarIdiomaTema(temaId: string) {
    if (!selectedIdioma) return;
    estudiarIdiomaTema(selectedIdioma, temaId);
    showToast("Concentración registrada");
  }

  function handleEstudiarAsigTema(temaId: string) {
    if (!selectedCurso || !selectedAsignatura) return;
    estudiarAsignaturaTema(selectedCurso, selectedAsignatura, temaId);
    showToast("Concentración registrada");
  }

  function handleProgresaFaseIdioma(temaId: string, currentFase?: TemaFase) {
    if (!selectedIdioma) return;
    const faseOrder: TemaFase[] = ["no_visto", "aprendiendo", "practicando", "repasando", "dominado"];
    const idx = faseOrder.indexOf(currentFase || "no_visto");
    const nextFase = faseOrder[Math.min(idx + 1, faseOrder.length - 1)];
    updateIdiomaTema(selectedIdioma, temaId, { fase: nextFase, completado: nextFase === "dominado" });
    showToast("Fase progresada: " + FASE_LABELS[nextFase]);
  }

  function handleProgresaFaseAsig(temaId: string, currentFase?: TemaFase) {
    if (!selectedCurso || !selectedAsignatura) return;
    const faseOrder: TemaFase[] = ["no_visto", "aprendiendo", "practicando", "repasando", "dominado"];
    const idx = faseOrder.indexOf(currentFase || "no_visto");
    const nextFase = faseOrder[Math.min(idx + 1, faseOrder.length - 1)];
    updateAsignaturaTema(selectedCurso, selectedAsignatura, temaId, { fase: nextFase, completado: nextFase === "dominado" });
    showToast("Fase progresada: " + FASE_LABELS[nextFase]);
  }

  function handleAddTemaIdi() {
    if (!selectedIdioma || !newTemaIdi.trim()) return;
    addIdiomaTema(selectedIdioma, newTemaIdi.trim(), newTemaIdiHabilidad);
    setNewTemaIdi("");
  }
  ...
  return (
    <>
              ...
              {tab === "idiomas" && (
                <>
                      ...
                      {idiomas.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {idiomas.map((idi) => {
                            ...
                            const urgentes = idi.temas.filter((t) => getIdiomaAutoPriority(t) === "alta").length;
                            const dias = diasHastaExamen(idi.fechaExamen);
                            const habStats = getIdiomaHabilidadStats(idi.temas);
                            const habilidadesActivas = Object.keys(habStats).length;
                            return (
                              <div
                                key={idi.id}
                              ...
                              >
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={`font-mono text-[12px] font-bold ${pct >= 70 ? "text-[#4ade80]" : pct >= 40 ? "text-[#fbbf24]" : "text-[#ef4444]"}`}>
                                      {pct}%
                                    </span>
                                  </div>
                                </div>
                                <div className="flex items-center gap-3 font-mono text-[11px] text-[#4a5a4a] mb-2">
                                  {idi.idioma && <span>{idi.idioma}</span>}
                                  {idi.nivel && <span>Nivel {idi.nivel}</span>}
                                  {dias !== null && (
                                    <span className={dias < 14 ? "text-[#ef4444]" : ""}>
                                      {dias > 0 ? `${dias}d` : dias === 0 ? "¡Hoy!" : "Vencido"}
                                    </span>
                                  )}
                                </div>
                                {/* Habilidad breakdown chips */}
                                {Object.keys(habStats).length > 0 && (
                                  <div className="flex flex-wrap gap-1 mb-1.5">
                                    {Object.entries(habStats).map(([h, s]) => (
                                      <span key={h} className={`font-mono text-[9px] px-1.5 py-0.5 rounded border ${HABILIDAD_BG_COLORS[h as IdiomaHabilidad] ?? "bg-[rgba(20,20,20,0.5)] border-[rgba(var(--mode-accent-rgb),0.15)]"} ${HABILIDAD_COLORS[h as IdiomaHabilidad] ?? "text-[#6b8a6b]"}`}>
                                        {HABILIDAD_LABELS[h as TemaIdiomaHabilidad] ?? h} {s.dominados}/{s.total}
                                      </span>
                                    ))}
                                  </div>
                                )}
                  ...
                  {selectedIdioma && editIdiForm && selectedIdi && (() => {
                    const pct = calcIdiomaProgress(selectedIdi);
                    const urgentes = selectedIdi.temas.filter((t) => getIdiomaAutoPriority(t) === "alta");
                    const habStats = getIdiomaHabilidadStats(selectedIdi.temas);
                    const habilidadesActivas = Object.keys(habStats).length;
                    const suggestedAction = getIdiomaSuggestedAction(selectedIdi);
                    const dias = diasHastaExamen(selectedIdi.fechaExamen);
                    const dominados = selectedIdi.temas.filter((t) => t.completado || t.fase === "dominado").length;
                    // Sort temas by auto priority
                    const sortedTemas = [...selectedIdi.temas].sort((a, b) => {
                      const pa = getIdiomaAutoPriority(a);
                      const pb = getIdiomaAutoPriority(b);
                      const pOrder = { alta: 0, media: 1, baja: 2 };
                      if (pOrder[pa] !== pOrder[pb]) return pOrder[pa] - pOrder[pb];
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-3">
                            <div className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider flex items-center justify-between">
                              ...
                              <button
                                onClick={() => setEditIdiomaDialogOpen(true)}
                                className="flex items-center gap-1 px-2 py-0.5 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#4a5a4a] hover:text-[var(--mode-accent)] hover:border-[rgba(var(--mode-accent-rgb),0.4)] transition-colors text-[10px]"
                              >
                                <Pencil className="w-3 h-3" />
                                Editar
                            ...
                            <div className="space-y-1">
                              {editIdiForm.idioma && (
                                <div className="font-mono text-[11px] text-[#4a5a4a] flex items-center gap-1.5">
                                  ...
                                  <span className="text-[#c8d6c0]">{editIdiForm.idioma}</span>
                                </div>
                              )}
                              {editIdiForm.nivel && (
                                <div className="font-mono text-[11px] text-[#4a5a4a] flex items-center gap-1.5">
                                  ...
                                  <span>Nivel</span>
                                  <span className="text-[#c8d6c0]">{editIdiForm.nivel}</span>
                                </div>
                              )}
                              {editIdiForm.fechaExamen && (
                                <div className="font-mono text-[11px] text-[#4a5a4a] flex items-center gap-1.5">
                                  <Calendar className="w-3 h-3 shrink-0" />
                                  <span className={dias !== null && dias < 14 ? "text-[#ef4444]" : "text-[#c8d6c0]"}>
                                    {formatDate(editIdiForm.fechaExamen)}
                                  </span>
                                  {dias !== null && (
                                    <span className={dias < 14 ? "text-[#ef4444]" : "text-[#4a5a4a]"}>
                                      ({dias > 0 ? `${dias}d` : dias === 0 ? "¡Hoy!" : "Vencido"})
                                    </span>
                                  )}
                                </div>
                          ...
                          <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(var(--mode-accent-rgb),0.1)] rounded px-3 py-2.5 space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">Progreso</div>
                                <div className={`font-mono text-[18px] font-bold leading-none ${pct >= 70 ? "text-[#4ade80]" : pct >= 40 ? "text-[#fbbf24]" : "text-[#ef4444]"}`}>{pct}%</div>
                                <ProgressLine pct={pct} />
                              </div>
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">Urgentes</div>
                              ...
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">Habilidades</div>
                                <span className="font-mono text-[18px] font-bold text-[var(--mode-accent)] leading-none">{habilidadesActivas}</span>
                                <div className="font-mono text-[9px] text-[#4a5a4a]">{dominados}/{selectedIdi.temas.length}</div>
                              </div>
                            </div>

                            {/* Habilidad progress bars */}
                            {Object.keys(habStats).length > 0 && (
                              <div className="space-y-1 pt-1 border-t border-[rgba(var(--mode-accent-rgb),0.06)]">
                                {Object.entries(habStats).map(([h, s]) => {
                                  const hPct = s.total > 0 ? Math.round((s.dominados / s.total) * 100) : 0;
                                  return (
                                    <div key={h} className="flex items-center gap-2">
                                      <span className={`font-mono text-[9px] w-20 shrink-0 ${HABILIDAD_COLORS[h as IdiomaHabilidad] ?? "text-[#6b8a6b]"}`}>
                                        {HABILIDAD_LABELS[h as TemaIdiomaHabilidad] ?? h}
                                      </span>
                                      <div className="flex-1 h-1 rounded-full bg-[rgba(20,20,20,0.5)] overflow-hidden">
                                        <div className="h-full rounded-full bg-[rgba(var(--mode-accent-rgb),0.4)] transition-all duration-300" style={{ width: hPct + "%" }} />
                                      </div>
                                      <span className="font-mono text-[9px] text-[#4a5a4a] w-7 text-right">{s.dominados}/{s.total}</span>
                                    </div>
                                  );
                                })}
                              </div>
                        ...
                        <div className="pt-1">
                          <div className="bg-[rgba(var(--mode-accent-rgb),0.06)] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-4 py-3 mb-3 flex items-center gap-2">
                            ...
                            <span className="ml-auto text-[10px] text-[#4a5a4a] normal-case tracking-normal flex items-center gap-1">
                              ordenado por prioridad automática
                              <span className="cursor-help" data-tooltip="Alta: no visto + sin estudio > 7d o sin estudio > 14d. Media: en progreso. Baja: dominado o reciente.">?</span>
                            </span>
                          </div>

                          {/* Add tema with habilidad selector */}
                          <div className="flex gap-2 mb-2">
                            <input
                              value={newTemaIdi}
                              onChange={(e) => setNewTemaIdi(e.target.value)}
                              onKeyDown={(e) => { if (e.key === "Enter" && newTemaIdi.trim()) handleAddTemaIdi(); }}
                              placeholder="Nuevo tema..."
                              className={`${inputClass} flex-1`}
                            />
                            <select
                              value={newTemaIdiHabilidad}
                              onChange={(e) => setNewTemaIdiHabilidad(e.target.value as IdiomaHabilidad)}
                              className="shrink-0 bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-2 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)] appearance-none cursor-pointer"
                            >
                              {Object.entries(HABILIDAD_LABELS).map(([k, v]) => (
                                <option key={k} value={k}>{v}</option>
                              ))}
                            </select>
                            <button
                              onClick={() => { if (newTemaIdi.trim()) handleAddTemaIdi(); }}
                          ...
                          {sortedTemas.length > 0 && (
                            <div className="space-y-1.5">
                              {sortedTemas.map((tema) => {
                                const priority = getIdiomaAutoPriority(tema);
                                const isCompleted = tema.completado || tema.fase === "dominado";
                                const isUrgent = priority === "alta" && (!tema.fechaUltimoEstudio || tema.fase === "no_visto");
                                const diasSinEstudio = tema.fechaUltimoEstudio
                                  ? Math.floor((Date.now() - new Date(tema.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000)
                                  : null;
                                const habilidad = (tema.habilidad || "vocabulario") as IdiomaHabilidad;
                                  ...
                                  <div
                                    ...
                                    className={`bg-[rgba(20,20,20,0.4)] border rounded px-3 py-2.5 transition-colors ${
                                      isUrgent
                                        ? "border-[rgba(239,68,68,0.25)] bg-[rgba(239,68,68,0.03)]"
                                        : isCompleted
                                          ? "border-[rgba(74,222,128,0.15)] opacity-70"
                                          : "border-[rgba(var(--mode-accent-rgb),0.1)]"
                                    }`}
                                  >
                                    ...
                                    <div className="flex items-center justify-between mb-1.5">
                                      <div className="flex items-center gap-2 flex-1 min-w-0">
                                        {isCompleted && <Trophy className="w-3.5 h-3.5 text-[#4ade80] shrink-0" />}
                                        <span className={`font-mono text-[14px] truncate ${isCompleted ? "text-[#4a5a4a] line-through" : isUrgent ? "text-[#ef4444]" : "text-[#c8d6c0]"}`}>{tema.texto}</span>
                                        {/* Habilidad badge */}
                                        <span className={`shrink-0 font-mono text-[9px] px-1.5 py-0.5 rounded border ${HABILIDAD_BG_COLORS[habilidad]} ${HABILIDAD_COLORS[habilidad]}`}>
                                          {HABILIDAD_LABELS[tema.habilidad || "vocabulario"]}
                                        </span>
                                      </div>
                                      <div className="flex items-center gap-1.5 shrink-0 ml-2">
                                        {/* Auto priority badge */}
                                    ...
                                    {/* Row 2: Collapsed stats */}
                                    <details className="group/stats">
                                      <summary className="font-mono text-[11px] text-[#4a5a4a] cursor-pointer hover:text-[#6b8a6b] transition-colors flex items-center gap-2 select-none mb-1.5">
                                        <span className="flex items-center gap-1">
                                          {tema.fase && (
                                            <span className="flex items-center gap-0.5">
                                              {FASE_ICONS[tema.fase]}
                                              {FASE_LABELS[tema.fase]}
                                            </span>
                                          )}
                                          {diasSinEstudio !== null && (
                                            <span className={`ml-2 ${diasSinEstudio > 7 ? "text-[#ef4444]" : ""}`}>
                                              {diasSinEstudio === 0 ? "Hoy" : `${diasSinEstudio}d`}
                                            </span>
                                          )}
                                          {!tema.fechaUltimoEstudio && (
                                            <span className="ml-2 text-[#ef4444]">Nunca estudiado</span>
                                          )}
                                        </span>
                                        <span className="text-[#3a4a3a] group-open/stats:rotate-180 transition-transform inline-block">▸</span>
                                      </summary>
                                      <div className="flex items-center gap-3 mb-2 font-mono text-[11px] text-[#4a5a4a] flex-wrap pl-4 border-l border-[rgba(var(--mode-accent-rgb),0.1)] ml-1 py-1">
                                        ...
                                        {diasSinEstudio !== null && (
                                          <span className={`flex items-center gap-1 ${diasSinEstudio > 7 ? "text-[#ef4444]" : ""}`}>
                                            <Clock className="w-2.5 h-2.5" />
                                            {diasSinEstudio === 0 ? "Hoy" : diasSinEstudio === 1 ? "Ayer" : `Hace ${diasSinEstudio}d`}
                                          </span>
                                        )}
                                        {!tema.fechaUltimoEstudio && (
                                          <span className="flex items-center gap-1">
                                    ...
                                    {!isCompleted && (
                                      <div className="flex items-center gap-2">
                                        <button
                                          onClick={() => {
                                            handleEstudiarIdiomaTema(tema.id);
                                            openMissionModal(`${editIdiForm.idioma || editIdiForm.nombre}: ${tema.texto}`, "oposicion");
                                          }}
                                          className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#60a5fa] border-[rgba(96,165,250,0.25)] hover:bg-[rgba(96,165,250,0.1)]"
                                        >
                                          <BookMarked className="w-2.5 h-2.5" />
                                        ...
                                        <button
                                          onClick={() => handleProgresaFaseIdioma(tema.id, tema.fase)}
                                          className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#fbbf24] border-[rgba(251,191,36,0.25)] hover:bg-[rgba(251,191,36,0.1)]"
                                        >
                                          <Brain className="w-2.5 h-2.5" />
                                          PROGRESAR FASE
                                        ...
                                        <button
                                          onClick={() => updateIdiomaTema(selectedIdioma!, tema.id, { fase: "dominado", completado: true })}
                                          className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#4ade80] border-[rgba(74,222,128,0.25)] hover:bg-[rgba(74,222,128,0.1)]"
                                        >
                          ...
                          {sortedTemas.length === 0 && (
                            <div className="text-center py-5 space-y-2">
                              <div className="font-mono text-[11px] text-[#4a5a4a] space-y-1.5">
                                <div className="flex items-center justify-center gap-2">
                                  <span className="w-4 h-4 rounded-full bg-[rgba(var(--mode-accent-rgb),0.08)] border border-[rgba(var(--mode-accent-rgb),0.15)] text-[9px] text-[var(--mode-accent)] flex items-center justify-center shrink-0">1</span>
                                  <span>Añade temas agrupados por habilidad (Vocabulario, Gramática...)</span>
                                </div>
                                <div className="flex items-center justify-center gap-2">
                                  <span className="w-4 h-4 rounded-full bg-[rgba(var(--mode-accent-rgb),0.08)] border border-[rgba(var(--mode-accent-rgb),0.15)] text-[9px] text-[var(--mode-accent)] flex items-center justify-center shrink-0">2</span>
                                  <span>Estudia cada tema y registra tu concentración</span>
                                </div>
                                <div className="flex items-center justify-center gap-2">
                                  <span className="w-4 h-4 rounded-full bg-[rgba(var(--mode-accent-rgb),0.08)] border border-[rgba(var(--mode-accent-rgb),0.15)] text-[9px] text-[var(--mode-accent)] flex items-center justify-center shrink-0">3</span>
                                  <span>Progresar fase hasta dominar cada tema</span>
                                </div>
                              </div>
                            </div>
                          )}
                        ...
                        <div className="flex gap-2 shrink-0 pt-2 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                          <button onClick={() => { setSelectedIdioma(null); setEditIdiForm(null); }} className="flex-1 font-mono text-[12px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#6b8a6b] hover:text-[#c8d6c0] transition-colors">Volver</button>
                          <button onClick={() => setDeleteConfirm({ type: "idioma", id: selectedIdioma! })} className="shrink-0 font-mono text-[10px] py-1.5 px-3 rounded border border-[rgba(239,68,68,0.2)] text-[#6b4a4a] hover:text-[#ef4444] hover:border-[rgba(239,68,68,0.3)] transition-colors flex items-center gap-1" data-tooltip="Eliminar idioma">
                            <Trash2 className="w-3 h-3" />
                            Eliminar
                          </button>
                        </div>
              ...
              {tab === "cursos" && (
                <>
                          ...
                          {cursos.map((c) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-3 font-mono text-[10px] text-[#4a5a4a] mb-2">
                                  <span className="text-[#4ade80]">{stats.aprobadas} aprobadas</span>
                                  <span className="text-[#60a5fa]">{stats.cursando} cursando</span>
                                  {stats.notaMedia !== null && (
                                    <span className={stats.notaMedia >= 5 ? "text-[#4ade80]" : "text-[#ef4444]"}>
                                      Nota media {stats.notaMedia.toFixed(1)}
                                    </span>
                                  )}
                                  {stats.ectsSuperados > 0 && (
                                    <span>{stats.ectsSuperados}/{stats.totalECTS} ECTS</span>
                                  )}
                                </div>
                                <ProgressLine pct={pct} />
                              </div>
                  ...
                  {selectedCurso && !selectedAsignatura && !showNewAsignatura && editCurForm && selectedCur && (() => {
                    const stats = calcCursoStats(selectedCur);
                    return (
                      <div className="space-y-5">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-3">
                            <div className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider flex items-center justify-between">
                              <span className="flex items-center gap-1.5"><FileText className="w-3 h-3" />Datos</span>
                              <button onClick={() => setEditCurDialogOpen(true)} className="flex items-center gap-1 px-2 py-0.5 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#4a5a4a] hover:text-[var(--mode-accent)] hover:border-[rgba(var(--mode-accent-rgb),0.4)] transition-colors text-[10px]"><Pencil className="w-3 h-3" />Editar</button>
                            </div>
                            <div className="space-y-1">
                              {editCurForm.institucion && (
                                <div className="font-mono text-[11px] text-[#4a5a4a] flex items-center gap-1.5"><Building2 className="w-3 h-3 shrink-0" /><span className="text-[#c8d6c0]">{editCurForm.institucion}</span></div>
                              )}
                              <div className="font-mono text-[11px] text-[#4a5a4a] flex items-center gap-1.5"><School className="w-3 h-3 shrink-0" /><span>Tipo</span><span className="text-[#c8d6c0]">{editCurForm.tipo}</span></div>
                            </div>
                          </div>
                          <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(var(--mode-accent-rgb),0.1)] rounded px-3 py-2.5 space-y-2">
                            <div className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider flex items-center gap-1.5"><BarChart3 className="w-3 h-3" />Resumen</div>
                            <div className="grid grid-cols-3 gap-3">
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">Aprobadas</div>
                                <span className="font-mono text-[18px] font-bold text-[#4ade80] leading-none">{stats.aprobadas}</span>
                                <div className="font-mono text-[9px] text-[#4a5a4a]">/{stats.total}</div>
                              </div>
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">ECTS</div>
                                <span className="font-mono text-[18px] font-bold text-[var(--mode-accent)] leading-none">{stats.ectsSuperados}</span>
                                <div className="font-mono text-[9px] text-[#4a5a4a]">/{stats.totalECTS}</div>
                              </div>
                              <div>
                                <div className="font-mono text-[9px] text-[#4a5a4a] uppercase mb-0.5">Nota media</div>
                                {stats.notaMedia !== null ? (
                                  <span className={`font-mono text-[18px] font-bold leading-none ${stats.notaMedia >= 5 ? "text-[#4ade80]" : "text-[#ef4444]"}`}>{stats.notaMedia.toFixed(1)}</span>
                                ) : (
                                  <span className="font-mono text-[18px] font-bold text-[#4a5a4a] leading-none">---</span>
                                )}
                              </div>
                            ...
                            {stats.enRiesgo > 0 && (
                              <div className="bg-[rgba(239,68,68,0.06)] border border-[rgba(239,68,68,0.15)] rounded px-2 py-1.5 flex items-start gap-1.5">
                                <AlertTriangle className="w-3 h-3 text-[#ef4444] shrink-0 mt-0.5" />
                                <span className="font-mono text-[10px] text-[#ef4444]">{stats.enRiesgo} asignatura{stats.enRiesgo > 1 ? "s" : ""} en riesgo</span>
                              </div>
                            )}
                            <div className="flex items-center gap-2 font-mono text-[9px] text-[#4a5a4a] flex-wrap">
                              <span className="text-[#4ade80]">{stats.aprobadas} aprobadas</span>
                              <span>·</span>
                              <span className="text-[#60a5fa]">{stats.cursando} cursando</span>
                              <span>·</span>
                              <span className="text-[#ef4444]">{stats.suspensas} suspensas</span>
                              <span>·</span>
                        ...
                        <div className="border-t border-[rgba(var(--mode-accent-rgb),0.1)] pt-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="font-mono text-[12px] text-[#6b8a6b] uppercase tracking-wider flex items-center gap-1.5"><BookOpen className="w-3 h-3" />ASIGNATURAS ({selectedCur.asignaturas.length})</div>
                            <button onClick={() => { setNewAsigForm({ ...EMPTY_ASIGNATURA }); setShowNewAsignatura(true); }} className="flex items-center gap-1 px-2 py-1 rounded border font-mono text-[11px] text-[var(--mode-accent)] border-[rgba(var(--mode-accent-rgb),0.3)] hover:bg-[rgba(var(--mode-accent-rgb),0.12)] transition-all"><Plus className="w-3 h-3" />NUEVA</button>
                          </div>
                          {selectedCur.asignaturas.length === 0 ? (
                            ...
                            <div className="space-y-1.5">
                              {selectedCur.asignaturas.map((asig) => {
                                ...
                                const dias = diasHastaExamen(asig.fechaExamen);
                                const isUrgent = asig.estado === "Cursando" && dias !== null && dias < 14 && dias >= 0 && pct < 40;
                                return (
                                  <div key={asig.id} onClick={() => openAsignaturaDetail(asig.id)} className={`bg-[rgba(20,20,20,0.4)] border rounded px-3 py-2.5 cursor-pointer transition-colors duration-150 hover:bg-[rgba(var(--mode-accent-rgb),0.08)] hover:border-[rgba(var(--mode-accent-rgb),0.15)] ${isUrgent ? "border-[rgba(239,68,68,0.25)] bg-[rgba(239,68,68,0.03)]" : "border-[rgba(var(--mode-accent-rgb),0.1)]"}`}>
                                    <div className="flex items-center justify-between mb-1">
                                      <div className="flex items-center gap-2 flex-1 min-w-0">
                                        <span className="shrink-0">{ESTADO_ICONS[asig.estado]}</span>
                                        <span className="font-mono text-[14px] text-[#c8d6c0] truncate">{asig.nombre}</span>
                                      </div>
                                      <div className="flex items-center gap-2 shrink-0 ml-2">
                                        <span className={`font-mono text-[11px] ${ESTADO_COLORS[asig.estado] ?? "text-[#6b8a6b]"}`}>{asig.estado}</span>
                                        <span className={`font-mono text-[11px] font-bold ${pct >= 70 ? "text-[#4ade80]" : pct >= 40 ? "text-[#fbbf24]" : "text-[#ef4444]"}`}>{pct}%</span>
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-3 font-mono text-[11px] text-[#4a5a4a] mb-1.5 flex-wrap">
                                      <span>{asig.creditos} ECTS</span>
                                      {notaFinal !== "---" && <span className={parseFloat(notaFinal) >= 5 ? "text-[#4ade80]" : "text-[#ef4444]"}>Nota: {notaFinal}</span>}
                                      {dias !== null && <span className={dias < 14 && dias >= 0 ? "text-[#ef4444]" : ""}>{dias > 0 ? `${dias}d` : dias === 0 ? "¡Hoy!" : "Vencido"}</span>}
                                      {isUrgent && <span className="text-[#ef4444] flex items-center gap-0.5 text-[10px]"><AlertTriangle className="w-3 h-3" />Urgente</span>}
                                    </div>
                                    <ProgressLine pct={pct} />
                                  </div>
                                );
                        ...
                        <div className="flex gap-2 shrink-0 pt-2 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                          <button onClick={() => { setSelectedCurso(null); setEditCurForm(null); }} className="flex-1 font-mono text-[12px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#6b8a6b] hover:text-[#c8d6c0] transition-colors">Volver</button>
                          <button onClick={() => setDeleteConfirm({ type: "curso", id: selectedCurso! })} className="shrink-0 font-mono text-[10px] py-1.5 px-3 rounded border border-[rgba(239,68,68,0.2)] text-[#6b4a4a] hover:text-[#ef4444] hover:border-[rgba(239,68,68,0.3)] transition-colors flex items-center gap-1"><Trash2 className="w-3 h-3" />Eliminar</button>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {selectedCurso && !selectedAsignatura && showNewAsignatura && (
                    <div className="space-y-3">
                      ...
                      <div><label className={labelClass}><BookOpen className="w-3 h-3" /> Nombre</label><input value={newAsigForm.nombre} onChange={(e) => setNewAsigForm((f) => ({ ...f, nombre: e.target.value }))} className={inputClass} placeholder="Nombre de la asignatura" /></div>
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className={labelClass}>Créditos</label><input value={newAsigForm.creditos} onChange={(e) => setNewAsigForm((f) => ({ ...f, creditos: parseInt(e.target.value) || 0 }))} className={inputClass} type="number" min={0} /></div>
                        <div><label className={labelClass}>Estado</label><select value={newAsigForm.estado} onChange={(e) => setNewAsigForm((f) => ({ ...f, estado: e.target.value as Asignatura["estado"] }))} className={selectClass}><option>Pendiente</option><option>Cursando</option><option>Aprobada</option><option>Suspensa</option><option>Convalidada</option></select></div>
                      </div>
                      <div><label className={labelClass}><Calendar className="w-3 h-3" /> Fecha examen</label><input value={newAsigForm.fechaExamen} onChange={(e) => setNewAsigForm((f) => ({ ...f, fechaExamen: e.target.value }))} className={inputClass} type="date" /></div>
                      <div className="flex gap-2 shrink-0 pt-2">
                        ...
                        <button onClick={handleAddAsignatura} disabled={!newAsigForm.nombre.trim()} className="flex-1 font-mono text-[12px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.3)] text-[var(--mode-accent)] bg-[rgba(var(--mode-accent-rgb),0.08)] transition-colors disabled:opacity-30 hover:bg-[rgba(var(--mode-accent-rgb),0.15)]"><Plus className="w-3 h-3 inline mr-1" />Crear</button>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedCurso && selectedAsignatura && editAsigForm && selectedAsig && (() => {
                    const notaFinal = calcNotaFinal(selectedAsig.evaluaciones);
                    const pct = calcAsignaturaProgress(selectedAsig);
                    const dias = diasHastaExamen(selectedAsig.fechaExamen);
                    const memorizarTemas = selectedAsig.temas.filter((t) => t.tipo === "Memorizar");
                    const trabajoTemas = selectedAsig.temas.filter((t) => t.tipo === "Trabajo");
                    const sortedMemorizar = [...memorizarTemas].sort((a, b) => {
                      const pa = getTemaAsigAutoPriority(a);
                      const pb = getTemaAsigAutoPriority(b);
                      const pOrder = { alta: 0, media: 1, baja: 2 };
                      if (pOrder[pa] !== pOrder[pb]) return pOrder[pa] - pOrder[pb];
                      return 0;
                    });
                    return (
                      <div className="space-y-5">
                        <div className="bg-[rgba(20,20,20,0.4)] border border-[rgba(var(--mode-accent-rgb),0.1)] rounded px-4 py-3 space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">{ESTADO_ICONS[selectedAsig.estado]}<span className="font-mono text-[14px] text-[#c8d6c0]">{editAsigForm.nombre}</span><span className={`font-mono text-[11px] ${ESTADO_COLORS[selectedAsig.estado] ?? "text-[#6b8a6b]"}`}>{editAsigForm.estado}</span></div>
                            <button onClick={() => setEditAsigDialogOpen(true)} className="flex items-center gap-1 px-2 py-0.5 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#4a5a4a] hover:text-[var(--mode-accent)] hover:border-[rgba(var(--mode-accent-rgb),0.4)] transition-colors text-[10px]"><Pencil className="w-3 h-3" />Editar</button>
                          </div>
                          <div className="flex items-center gap-4 font-mono text-[11px] text-[#4a5a4a]">
                            <span>{editAsigForm.creditos} ECTS</span>
                            {notaFinal !== "---" && <span className={parseFloat(notaFinal) >= 5 ? "text-[#4ade80]" : "text-[#ef4444]"}>Nota: {notaFinal}</span>}
                            {editAsigForm.fechaExamen && <span className={dias !== null && dias < 14 ? "text-[#ef4444]" : ""}><Calendar className="w-3 h-3 inline mr-0.5" />{formatDate(editAsigForm.fechaExamen)}{dias !== null && <span className="ml-1">({dias > 0 ? `${dias}d` : dias === 0 ? "¡Hoy!" : "Vencido"})</span>}</span>}
                            <span className={`ml-auto font-bold ${pct >= 70 ? "text-[#4ade80]" : pct >= 40 ? "text-[#fbbf24]" : "text-[#ef4444]"}`}>{pct}%</span>
                          </div>
                          <ProgressLine pct={pct} />
                        </div>
                        ...
                        <div className="border-t border-[rgba(var(--mode-accent-rgb),0.1)] pt-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="font-mono text-[12px] text-[#6b8a6b] uppercase tracking-wider flex items-center gap-1.5"><BarChart3 className="w-3 h-3" />EVALUACIONES ({selectedAsig.evaluaciones.length})</div>
                            <button onClick={addNewEvaluacion} className="flex items-center gap-1 px-2 py-1 rounded border font-mono text-[11px] text-[var(--mode-accent)] border-[rgba(var(--mode-accent-rgb),0.3)] hover:bg-[rgba(var(--mode-accent-rgb),0.12)] transition-all"><Plus className="w-3 h-3" />Añadir</button>
                          </div>
                          <div className="flex items-center gap-3 mb-2">
                            <span className="font-mono text-[12px] text-[#6b8a6b]">Nota final:</span>
                            <span className={`font-mono text-[16px] font-bold ${notaFinal === "---" ? "text-[#4a5a4a]" : parseFloat(notaFinal) >= 5 ? "text-[#4ade80]" : "text-[#ef4444]"}`}>{notaFinal}</span>
                          </div>
                          {(() => { const n = calcNeededForPass(selectedAsig.evaluaciones); return n !== null ? (<div className={`font-mono text-[11px] mb-2 ${n <= 10 ? "text-[#fbbf24]" : "text-[#ef4444]"}`}>Para aprobar necesitas sacar un {n.toFixed(2)} de media en las restantes</div>) : null; })()}
                          {selectedAsig.evaluaciones.length > 0 && (
                            <div className="overflow-x-auto">
                              <table className="w-full text-left">
                                <thead><tr className="border-b border-[rgba(var(--mode-accent-rgb),0.15)]"><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal">Concepto</th><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal w-20">Nota</th><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal w-20">Peso %</th><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal w-20">Aporte</th><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal w-24">Fecha</th><th className="font-mono text-[11px] text-[#6b8a6b] uppercase tracking-wider py-2 px-2 font-normal w-16"></th></tr></thead>
                                <tbody>{selectedAsig.evaluaciones.map((ev) => {
                                  const isEditing = editingEvalId === ev.id;
                                  const aporte = ev.nota !== null ? (ev.nota * ev.peso / 100).toFixed(2) : "---";
                                  return (<tr key={ev.id} className="border-b border-[rgba(var(--mode-accent-rgb),0.1)]">
                                    {isEditing ? (<><td className="py-1.5 px-1"><input value={editEvalData.concepto} onChange={(e) => setEditEvalData((d) => ({ ...d, concepto: e.target.value }))} className="w-full bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-1 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)]" /></td><td className="py-1.5 px-1"><input value={editEvalData.nota} onChange={(e) => setEditEvalData((d) => ({ ...d, nota: e.target.value }))} className="w-full bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-1 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)]" type="number" step="0.01" placeholder="---" /></td><td className="py-1.5 px-1"><input value={editEvalData.peso} onChange={(e) => setEditEvalData((d) => ({ ...d, peso: e.target.value }))} className="w-full bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-1 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)]" type="number" min={0} /></td><td className="py-1.5 px-2 font-mono text-[12px] text-[#4a5a4a]">---</td><td className="py-1.5 px-1"><input value={editEvalData.fecha} onChange={(e) => setEditEvalData((d) => ({ ...d, fecha: e.target.value }))} className="w-full bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-1 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)]" type="date" /></td><td className="py-1.5 px-1"><div className="flex gap-1"><button onClick={saveEditEval} className="text-[#4ade80] hover:text-[#22c55e] transition-colors"><CheckCircle2 className="w-3.5 h-3.5" /></button><button onClick={() => setEditingEvalId(null)} className="text-[#4a5a4a] hover:text-[#c8d6c0] transition-colors"><X className="w-3.5 h-3.5" /></button></div></td></>) : (<><td className="py-2 px-2 font-mono text-[12px] text-[#c8d6c0]">{ev.concepto || "---"}</td><td className="py-2 px-2 font-mono text-[12px] text-[#6b8a6b]">{ev.nota !== null ? String(ev.nota) : "---"}</td><td className="py-2 px-2 font-mono text-[12px] text-[#6b8a6b]">{ev.peso}%</td><td className="py-2 px-2 font-mono text-[12px] text-[#4a5a4a]">{aporte}</td><td className="py-2 px-2 font-mono text-[12px] text-[#4a5a4a]">{ev.fecha ? formatDate(ev.fecha) : "---"}</td><td className="py-2 px-1"><div className="flex gap-1"><button onClick={() => startEditEval(ev)} className="text-[#4a5a4a] hover:text-[#6b8a6b] transition-colors"><Pencil className="w-3 h-3" /></button><button onClick={() => { setEditingEvalId(ev.id); setDeleteConfirm({ type: "evaluacion", id: selectedCurso!, extraId: selectedAsignatura! }); }} className="text-[#4a5a4a] hover:text-[#ef4444] transition-colors"><Trash2 className="w-3 h-3" /></button></div></td></>)}
                                  </tr>);
                                })}</tbody>
                              </table>
                            </div>
                          )}
                          {selectedAsig.evaluaciones.length > 0 && (<div className="font-mono text-[11px] text-[#4a5a4a] mt-2 flex items-center gap-2">Suma pesos: {calcSumPesos(selectedAsig.evaluaciones)}%{calcSumPesos(selectedAsig.evaluaciones) !== 100 && <span className="text-[#ef4444]">(!= 100%)</span>}</div>)}
                        </div>
                        ...
                        <div className="border-t border-[rgba(var(--mode-accent-rgb),0.1)] pt-4">
                          <div className="bg-[rgba(var(--mode-accent-rgb),0.06)] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-4 py-3 mb-3 flex items-center gap-2">
                            <BookOpen className="w-4 h-4 text-[var(--mode-accent)]" />
                            <span className="font-mono text-[13px] text-[var(--mode-accent)] uppercase tracking-wider font-bold">TEMARIO ({selectedAsig.temas.length})</span>
                            <span className="ml-auto text-[10px] text-[#4a5a4a] normal-case tracking-normal">{memorizarTemas.length} memorizar · {trabajoTemas.length} trabajos</span>
                          </div>
                          <div className="flex gap-2 mb-2">
                            <select value={newTemaAsigTipo} onChange={(e) => setNewTemaAsigTipo(e.target.value as TemaAsignaturaTipo)} className="shrink-0 bg-[rgba(20,20,20,0.5)] border border-[rgba(var(--mode-accent-rgb),0.25)] rounded px-2 py-2.5 font-mono text-[12px] text-[#c8d6c0] focus:outline-none focus:border-[rgba(var(--mode-accent-rgb),0.5)] appearance-none cursor-pointer"><option value="Memorizar">Memorizar</option><option value="Trabajo">Trabajo</option></select>
                            <input value={newTemaAsig} onChange={(e) => setNewTemaAsig(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && newTemaAsig.trim()) { addAsignaturaTema(selectedCurso!, selectedAsignatura!, newTemaAsig.trim(), newTemaAsigTipo); setNewTemaAsig(""); } }} placeholder={newTemaAsigTipo === "Memorizar" ? "Nuevo tema..." : "Nuevo trabajo..."} className={`${inputClass} flex-1`} />
                            <button onClick={() => { if (newTemaAsig.trim()) { addAsignaturaTema(selectedCurso!, selectedAsignatura!, newTemaAsig.trim(), newTemaAsigTipo); setNewTemaAsig(""); } }} disabled={!newTemaAsig.trim()} className="shrink-0 text-[#4a5a4a] hover:text-[#c8d6c0] transition-colors disabled:opacity-30"><Plus className="w-4 h-4" /></button>
                          </div>
                          {(selectedAsig.temas.some((t) => t.tipo === "Memorizar" && getTemaAsigAutoPriority(t) === "alta") || selectedAsig.temas.some((t) => t.tipo === "Trabajo" && t.prioridad === "alta" && !t.completado)) && (
                            <div className="bg-[rgba(251,191,36,0.08)] border border-[rgba(251,191,36,0.2)] rounded px-3 py-2 mb-2 flex items-center gap-2"><Lightbulb className="w-3.5 h-3.5 text-[#fbbf24] shrink-0" /><span className="font-mono text-[11px] text-[#fbbf24]">SUGERENCIA: Empieza por los elementos de ALTA prioridad.</span></div>
                          )}
                          {selectedAsig.temas.length > 0 && (
                            <div className="space-y-1.5">
                              {sortedMemorizar.length > 0 && (<>
                                <div className="font-mono text-[10px] text-[#4a5a4a] uppercase tracking-wider mb-1 flex items-center gap-1"><BookMarked className="w-3 h-3" />Memorizar ({sortedMemorizar.length})</div>
                                {sortedMemorizar.map((tema) => {
                                  const isCompleted = tema.completado || tema.fase === "dominado";
                                  const priority = getTemaAsigAutoPriority(tema);
                                  const isUrgent = priority === "alta" && (!tema.fechaUltimoEstudio || tema.fase === "no_visto");
                                  const diasSinEstudio = tema.fechaUltimoEstudio ? Math.floor((Date.now() - new Date(tema.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000) : null;
                                  return (
                                    <div key={tema.id} className={`bg-[rgba(20,20,20,0.4)] border rounded px-3 py-2.5 transition-colors ${isUrgent ? "border-[rgba(239,68,68,0.25)] bg-[rgba(239,68,68,0.03)]" : isCompleted ? "border-[rgba(74,222,128,0.15)] opacity-70" : "border-[rgba(var(--mode-accent-rgb),0.1)]"}`}>
                                      <div className="flex items-center justify-between mb-1.5">
                                        <div className="flex items-center gap-2 flex-1 min-w-0"><BookMarked className="w-3 h-3 text-[#4a5a4a] shrink-0" />{isCompleted && <Trophy className="w-3.5 h-3.5 text-[#4ade80] shrink-0" />}<span className="font-mono text-[14px] text-[#c8d6c0] truncate">{tema.texto}</span></div>
                                        <div className="flex items-center gap-1.5 shrink-0 ml-2">
                                          {!isCompleted && <span className={`font-mono text-[10px] font-bold flex items-center gap-0.5 ${autoPriorityColor(priority)}`}><PrioridadIconAuto priority={priority} />{autoPriorityLabel(priority)}</span>}
                                          <button onClick={() => setDeleteConfirm({ type: "asignatura_tema", id: selectedCurso!, extraId: tema.id })} className="text-[#4a5a4a] hover:text-[#ef4444] transition-colors"><Trash2 className="w-3 h-3" /></button>
                                        </div>
                                      </div>
                                      <details className="group/stats">
                                        <summary className="font-mono text-[11px] text-[#4a5a4a] cursor-pointer hover:text-[#6b8a6b] transition-colors flex items-center gap-2 select-none mb-1.5">
                                          <span className="flex items-center gap-1">{tema.fase && <span className="flex items-center gap-0.5">{FASE_ICONS[tema.fase]}{FASE_LABELS[tema.fase]}</span>}{diasSinEstudio !== null && <span className={`ml-2 ${diasSinEstudio > 7 ? "text-[#ef4444]" : ""}`}>{diasSinEstudio === 0 ? "Hoy" : `${diasSinEstudio}d`}</span>}{!tema.fechaUltimoEstudio && <span className="ml-2 text-[#ef4444]">Nunca estudiado</span>}</span>
                                          <span className="text-[#3a4a3a] group-open/stats:rotate-180 transition-transform inline-block">▸</span>
                                        </summary>
                                        <div className="flex items-center gap-3 mb-2 font-mono text-[11px] text-[#4a5a4a] flex-wrap pl-4 border-l border-[rgba(var(--mode-accent-rgb),0.1)] ml-1 py-1">{tema.fase && <span className="flex items-center gap-1">{FASE_ICONS[tema.fase]}{FASE_LABELS[tema.fase]}</span>}{diasSinEstudio !== null && <span className={`flex items-center gap-1 ${diasSinEstudio > 7 ? "text-[#ef4444]" : ""}`}><Clock className="w-2.5 h-2.5" />{diasSinEstudio === 0 ? "Hoy" : diasSinEstudio === 1 ? "Ayer" : `Hace ${diasSinEstudio}d`}</span>}{!tema.fechaUltimoEstudio && <span className="flex items-center gap-1"><Eye className="w-2.5 h-2.5" />Nunca estudiado</span>}</div>
                                      </details>
                                      {!isCompleted && (
                                        <div className="flex items-center gap-2">
                                          <button onClick={() => { handleEstudiarAsigTema(tema.id); openMissionModal(`Estudiar: ${tema.texto}`, "oposicion"); }} className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#60a5fa] border-[rgba(96,165,250,0.25)] hover:bg-[rgba(96,165,250,0.1)]"><BookMarked className="w-2.5 h-2.5" />ESTUDIAR</button>
                                          <button onClick={() => handleProgresaFaseAsig(tema.id, tema.fase)} className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#fbbf24] border-[rgba(251,191,36,0.25)] hover:bg-[rgba(251,191,36,0.1)]"><Brain className="w-2.5 h-2.5" />PROGRESAR FASE</button>
                                          <button onClick={() => updateAsignaturaTema(selectedCurso!, selectedAsignatura!, tema.id, { fase: "dominado", completado: true })} className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#4ade80] border-[rgba(74,222,128,0.25)] hover:bg-[rgba(74,222,128,0.1)]"><Trophy className="w-2.5 h-2.5" />DOMINADO</button>
                                        </div>
                                      )}
                                    </div>
                                  );
                              ...
                              {trabajoTemas.length > 0 && (<>
                                <div className="font-mono text-[10px] text-[#4a5a4a] uppercase tracking-wider mb-1 mt-2 flex items-center gap-1"><ClipboardList className="w-3 h-3" />Trabajos ({trabajoTemas.length})</div>
                                {trabajoTemas.map((tema) => {
                                  const diasSinEstudio = tema.fechaUltimoEstudio ? Math.floor((Date.now() - new Date(tema.fechaUltimoEstudio + "T00:00:00").getTime()) / 86400000) : null;
                                  return (
                                    <div key={tema.id} className={`bg-[rgba(20,20,20,0.4)] border rounded px-3 py-2.5 transition-colors ${tema.completado ? "border-[rgba(74,222,128,0.15)] opacity-70" : "border-[rgba(var(--mode-accent-rgb),0.1)]"}`}>
                                      <div className="flex items-center justify-between mb-1.5">
                                        <div className="flex items-center gap-2 flex-1 min-w-0"><ClipboardList className="w-3 h-3 text-[#4a5a4a] shrink-0" />{tema.completado && <CheckCircle2 className="w-3.5 h-3.5 text-[#4ade80] shrink-0" />}<span className={`font-mono text-[14px] truncate ${tema.completado ? "text-[#4a5a4a] line-through" : "text-[#c8d6c0]"}`}>{tema.texto}</span></div>
                                        <button onClick={() => setDeleteConfirm({ type: "asignatura_tema", id: selectedCurso!, extraId: tema.id })} className="text-[#4a5a4a] hover:text-[#ef4444] transition-colors shrink-0 ml-2"><Trash2 className="w-3 h-3" /></button>
                                      </div>
                                      <details className="group/stats">
                                        <summary className="font-mono text-[11px] text-[#4a5a4a] cursor-pointer hover:text-[#6b8a6b] transition-colors flex items-center gap-2 select-none mb-1.5">
                                          <span className="flex items-center gap-1">{diasSinEstudio !== null && <span className={diasSinEstudio > 7 ? "text-[#ef4444]" : ""}>Última actividad: {diasSinEstudio === 0 ? "Hoy" : `${diasSinEstudio}d`}</span>}{!tema.fechaUltimoEstudio && <span className="text-[#ef4444]">Nunca trabajado</span>}</span>
                                          <span className="text-[#3a4a3a] group-open/stats:rotate-180 transition-transform inline-block">▸</span>
                                        </summary>
                                        <div className="flex items-center gap-3 mb-2 font-mono text-[11px] text-[#4a5a4a] flex-wrap pl-4 border-l border-[rgba(var(--mode-accent-rgb),0.1)] ml-1 py-1">{diasSinEstudio !== null && <span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" />{diasSinEstudio === 0 ? "Hoy" : diasSinEstudio === 1 ? "Ayer" : `Hace ${diasSinEstudio}d`}</span>}{!tema.fechaUltimoEstudio && <span className="flex items-center gap-1"><Eye className="w-2.5 h-2.5" />Nunca trabajado</span>}</div>
                                      </details>
                                      <div className="flex items-center gap-2 flex-wrap">
                                        <button onClick={() => { handleEstudiarAsigTema(tema.id); openMissionModal(`Trabajar: ${tema.texto}`, "oposicion"); }} className="font-mono text-[10px] flex items-center gap-1 px-2 py-1 rounded border transition-colors text-[#60a5fa] border-[rgba(96,165,250,0.25)] hover:bg-[rgba(96,165,250,0.1)]"><BookMarked className="w-2.5 h-2.5" />ESTUDIAR</button>
                                        <button onClick={() => openMissionModal(`Entender enunciado: ${tema.texto}`, "oposicion")} className="font-mono text-[10px] text-[#4a5a4a] hover:text-[#c8d6c0] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-1.5 py-0.5 transition-colors">Entender enunciado</button>
                                        <button onClick={() => openMissionModal(`Buscar información: ${tema.texto}`, "oposicion")} className="font-mono text-[10px] text-[#4a5a4a] hover:text-[#c8d6c0] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-1.5 py-0.5 transition-colors">Buscar información</button>
                                        <button onClick={() => openMissionModal(`Redactar borrador: ${tema.texto}`, "oposicion")} className="font-mono text-[10px] text-[#4a5a4a] hover:text-[#c8d6c0] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-1.5 py-0.5 transition-colors">Redactar borrador</button>
                                        <button onClick={() => openMissionModal(`Revisar final: ${tema.texto}`, "oposicion")} className="font-mono text-[10px] text-[#4a5a4a] hover:text-[#c8d6c0] border border-[rgba(var(--mode-accent-rgb),0.12)] rounded px-1.5 py-0.5 transition-colors">Revisar final</button>
                                        <button onClick={() => updateAsignaturaTema(selectedCurso!, selectedAsignatura!, tema.id, { completado: !tema.completado })} className={`font-mono text-[10px] border rounded px-1.5 py-0.5 transition-colors flex items-center gap-1 ${tema.completado ? "text-[#4ade80] border-[rgba(74,222,128,0.3)] bg-[rgba(74,222,128,0.08)]" : "text-[#4a5a4a] border-[rgba(var(--mode-accent-rgb),0.12)] hover:text-[#4ade80]"}`}><CheckCircle2 className="w-2.5 h-2.5" />Entregado</button>
                                      </div>
                                    </div>
                                  );
                                })}
                          ...
                          {selectedAsig.temas.length > 0 && (<div className="font-mono text-[12px] text-[#4a5a4a] mt-2">{selectedAsig.temas.filter((t) => t.completado || t.fase === "dominado").length}/{selectedAsig.temas.length} completados ({calcAsignaturaProgress(selectedAsig)}%)</div>)}
                        </div>

                        <div className="flex gap-2 shrink-0 pt-2 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                          <button onClick={() => { setSelectedAsignatura(null); setEditAsigForm(null); setEditingEvalId(null); }} className="flex-1 font-mono text-[12px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.2)] text-[#6b8a6b] hover:text-[#c8d6c0] transition-colors">Volver</button>
                          <button onClick={() => setDeleteConfirm({ type: "asignatura", id: selectedCurso!, extraId: selectedAsignatura! })} className="shrink-0 font-mono text-[10px] py-1.5 px-3 rounded border border-[rgba(239,68,68,0.2)] text-[#6b4a4a] hover:text-[#ef4444] hover:border-[rgba(239,68,68,0.3)] transition-colors flex items-center gap-1"><Trash2 className="w-3 h-3" />Eliminar</button>
                        </div>
                      </div>
                    );
                  })()}
      ...
      {/* ---- Edit Idioma Dialog ---- */}
      <Dialog open={editIdiomaDialogOpen} onOpenChange={setEditIdiomaDialogOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto rounded-lg backdrop-blur-md p-6 shadow-[0_0_30px_rgba(0,0,0,0.5)]" style={{ background: studyBg, borderColor: `rgba(${mc.accentRgb},0.2)` }}>
          <DialogHeader>
            <DialogTitle className="font-mono text-sm flex items-center gap-2" style={{ color: mc.accent }}><Languages className="w-4 h-4" />EDITAR IDIOMA</DialogTitle>
            <DialogDescription className="sr-only">Editar datos del idioma</DialogDescription>
          </DialogHeader>
          {editIdiForm && (
            <div className="space-y-3 mt-2">
              <div><label className={labelClass}><Languages className="w-3 h-3" /> Nombre</label><input value={editIdiForm.nombre} onChange={(e) => setEditIdiForm((f) => ({ ...f!, nombre: e.target.value }))} className={inputClass} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>Idioma</label><input value={editIdiForm.idioma} onChange={(e) => setEditIdiForm((f) => ({ ...f!, idioma: e.target.value }))} className={inputClass} /></div>
                <div><label className={labelClass}><BarChart3 className="w-3 h-3" /> Nivel</label><input value={editIdiForm.nivel} onChange={(e) => setEditIdiForm((f) => ({ ...f!, nivel: e.target.value }))} className={inputClass} /></div>
              </div>
              <div><label className={labelClass}><Calendar className="w-3 h-3" /> Fecha examen</label><input value={editIdiForm.fechaExamen} onChange={(e) => setEditIdiForm((f) => ({ ...f!, fechaExamen: e.target.value }))} className={inputClass} type="date" /></div>
              <div className="flex gap-2 mt-4 pt-3 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                <button onClick={() => setEditIdiomaDialogOpen(false)} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.15)] text-[#4a5a4a] hover:text-[#6b8a6b] transition-colors">Cancelar</button>
                <button onClick={() => { saveIdioma(); setEditIdiomaDialogOpen(false); }} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.4)] text-[var(--mode-accent)] bg-[rgba(var(--mode-accent-rgb),0.1)] hover:bg-[rgba(var(--mode-accent-rgb),0.2)] transition-colors">Guardar</button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      {/* ---- Edit Curso Dialog ---- */}
      <Dialog open={editCurDialogOpen} onOpenChange={setEditCurDialogOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto rounded-lg backdrop-blur-md p-6 shadow-[0_0_30px_rgba(0,0,0,0.5)]" style={{ background: studyBg, borderColor: `rgba(${mc.accentRgb},0.2)` }}>
          <DialogHeader>
            <DialogTitle className="font-mono text-sm flex items-center gap-2" style={{ color: mc.accent }}><School className="w-4 h-4" />EDITAR CURSO</DialogTitle>
            <DialogDescription className="sr-only">Editar datos del curso</DialogDescription>
          </DialogHeader>
          {editCurForm && (
            <div className="space-y-3 mt-2">
              <div><label className={labelClass}><School className="w-3 h-3" /> Nombre</label><input value={editCurForm.nombre} onChange={(e) => setEditCurForm((f) => ({ ...f!, nombre: e.target.value }))} className={inputClass} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>Institución</label><input value={editCurForm.institucion} onChange={(e) => setEditCurForm((f) => ({ ...f!, institucion: e.target.value }))} className={inputClass} /></div>
                <div><label className={labelClass}>Tipo</label><select value={editCurForm.tipo} onChange={(e) => setEditCurForm((f) => ({ ...f!, tipo: e.target.value as Curso["tipo"] }))} className={selectClass}><option>Grado</option><option>Master</option><option>Curso online</option><option>Certificación</option><option>Otro</option></select></div>
              </div>
              <div className="flex gap-2 mt-4 pt-3 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                <button onClick={() => setEditCurDialogOpen(false)} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.15)] text-[#4a5a4a] hover:text-[#6b8a6b] transition-colors">Cancelar</button>
                <button onClick={() => { saveCurso(); setEditCurDialogOpen(false); }} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.4)] text-[var(--mode-accent)] bg-[rgba(var(--mode-accent-rgb),0.1)] hover:bg-[rgba(var(--mode-accent-rgb),0.2)] transition-colors">Guardar</button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={editAsigDialogOpen} onOpenChange={setEditAsigDialogOpen}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto rounded-lg backdrop-blur-md p-6 shadow-[0_0_30px_rgba(0,0,0,0.5)]" style={{ background: studyBg, borderColor: `rgba(${mc.accentRgb},0.2)` }}>
          <DialogHeader>
            <DialogTitle className="font-mono text-sm flex items-center gap-2" style={{ color: mc.accent }}><BookOpen className="w-4 h-4" />EDITAR ASIGNATURA</DialogTitle>
            <DialogDescription className="sr-only">Editar datos de la asignatura</DialogDescription>
          </DialogHeader>
          {editAsigForm && (
            <div className="space-y-3 mt-2">
              <div><label className={labelClass}><BookOpen className="w-3 h-3" /> Nombre</label><input value={editAsigForm.nombre} onChange={(e) => setEditAsigForm((f) => ({ ...f!, nombre: e.target.value }))} className={inputClass} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>Créditos</label><input value={editAsigForm.creditos} onChange={(e) => setEditAsigForm((f) => ({ ...f!, creditos: parseInt(e.target.value) || 0 }))} className={inputClass} type="number" min={0} /></div>
                <div><label className={labelClass}>Estado</label><select value={editAsigForm.estado} onChange={(e) => setEditAsigForm((f) => ({ ...f!, estado: e.target.value as Asignatura["estado"] }))} className={selectClass}><option>Pendiente</option><option>Cursando</option><option>Aprobada</option><option>Suspensa</option><option>Convalidada</option></select></div>
              </div>
              <div><label className={labelClass}><Calendar className="w-3 h-3" /> Fecha examen</label><input value={editAsigForm.fechaExamen} onChange={(e) => setEditAsigForm((f) => ({ ...f!, fechaExamen: e.target.value }))} className={inputClass} type="date" /></div>
              <div className="flex gap-2 mt-4 pt-3 border-t border-[rgba(var(--mode-accent-rgb),0.1)]">
                <button onClick={() => setEditAsigDialogOpen(false)} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.15)] text-[#4a5a4a] hover:text-[#6b8a6b] transition-colors">Cancelar</button>
                <button onClick={() => { saveAsignatura(); setEditAsigDialogOpen(false); }} className="flex-1 font-mono text-[11px] py-2 rounded border border-[rgba(var(--mode-accent-rgb),0.4)] text-[var(--mode-accent)] bg-[rgba(var(--mode-accent-rgb),0.1)] hover:bg-[rgba(var(--mode-accent-rgb),0.2)] transition-colors">Guardar</button>
              </div>
            </div>
          )}
        </DialogContent>
